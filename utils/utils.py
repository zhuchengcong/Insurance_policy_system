...
def wechat_mini_login(code):
    ...
    get_user_url = 'https://api.weixin.qq.com/sns/jscode2session?appid={}&secret={}&js_code={}&grant_type=authorization_code'.format(
        appid, secret, code)
    ...
    if response_dic.get('errcode') and response_dic.get('errcode') != 0:
        return {"message": "微信端推送错误：%s,errcode=%s" % (response_dic.get('errmsg'), response_dic.get('errcode')),
                "errorCode": 1, "data": {}}
...
def get_wechat_token():
    ...
    get_user_url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={}&secret={}'.format(
        appid, secret)
    response = requests.get(url=get_user_url)
    # print('msg：',eval(response.text), type(eval(response.text)))
    response_dic = eval(response.text)
...
def wechat_app_login(code):
    ...
    get_user_url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid={}&secret={}&code={}&grant_type=authorization_code'.format(
        appid, secret, code)
    response = requests.get(url=get_user_url)
    print('msg：', eval(response.text), type(eval(response.text)))
    response_dic = eval(response.text)
    ...
    get_user_info = 'https://api.weixin.qq.com/sns/userinfo?access_token={}&openid={}&lang=zh_CN'.format(access_token,
                                                                                                         open_id)
...
class VisitThrottle(BaseThrottle):
    ...
    def allow_request(self, request, view):
        ...
        while history and history[-1] < ctime - 60:
            history.pop()
        if len(history) < 1000:  # 限制的频数 设置同一IP该接口一分钟内只能被访问100次
