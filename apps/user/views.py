    ...
    def post(self, request):
        ...
        try:
            ...
            if user.password == password:
                token_data = jwt_response_payload_handler(jwt_encode_handler(payload=jwt_payload_handler(user)), user,
                                                          request)
    ...
    def post(self, request):
        ...
        try:
            ...
            if we_user and app_user:
                ...
                user.region = ' '.join(
                    [str(item) for item in [userInfo.get('country'), userInfo.get('province'), userInfo.get('city')] if
                     item != None])
                user.save()
            token_data = jwt_response_payload_handler(jwt_encode_handler(payload=jwt_payload_handler(user)), user,
                                                      request)
    ...
    def post(self, request):
        ...
        try:
            ...
            if not user:
                ...
                user.region = ' '.join(
                    [str(item) for item in [user_info.get('country'), user_info.get('province'), user_info.get('city')]
                     if item != None])
                user.save()
    ...
    def post(self, request):
        ...
        try:
            ...
            if not user_obj:
                ...
                user_obj.save()
            token_data = jwt_response_payload_handler(jwt_encode_handler(jwt_payload_handler(user_obj)), user_obj,
                                                      request)
            ...
            cache.delete(mobile)
            return Response(
                {"message": "登录成功", "errorCode": 0, "data": {'token': token_data.get('token'), 'is_login': is_login}})
        except Exception as e:
            print('发生错误：', e)
            return Response({"message": "出现了无法预料的view视图错误：%s" % e, "errorCode": 1, "data": {}})
...
class UserViewset(ModelViewSet):
    ...
    list:  获取用户列表
    '''
    queryset = User.objects.filter(group__group_type__in=['SuperAdmin', 'Admin', 'NormalUser']).order_by('-create_time')
    authentication_classes = (JWTAuthentication,)
...
class UserGroupViewset(ModelViewSet):
    ...
    permission_classes = [BaseAuthPermission, ]
    throttle_classes = [VisitThrottle]
    serializer_class = UserUseGroupSerializer
    filter_backends = (DjangoFilterBackend, SearchFilter, OrderingFilter,)
    search_fields = ('group_type_cn',)
    ordering_fields = ('id', 'update_time', 'create_time',)
    pagination_class = Pagination
